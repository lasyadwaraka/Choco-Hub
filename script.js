...
// ===== Add-to-Cart POPUP (matches your HTML) =====
(function () {
  ...
  // Event delegation so it works for ALL "Add to Cart" buttons
  document.addEventListener("click", (e) => {
...
// ========== CART PANEL LOGIC (append at very end of script.js) ==========
document.addEventListener('DOMContentLoaded', () => {
  ...
  // change qty/remove inside the cart panel (event delegation)
  ...
  // build an item from the clicked card & add to cart
  function addToCartFromButton(btn){
    ...
    // price element on the card shows TOTAL (updates when qty changes)
    ...
    // merge if same item (by name + unit price)
  ...
  // hook into ALL add-to-cart buttons (keeps your existing popup)
  document.addEventListener('click', (e)=>{
    const btn = e.target.closest('.add_to_cart_btn');
    if (!btn) return;

    // keep your popup if it's defined elsewhere
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Open order panel when cart checkout button is clicked
  ...
  orderForm?.addEventListener('submit', (e) => {
    ...
    // Close order panel and reset form
    orderPanel.hidden = true;
    orderPanel.classList.remove('open');
    orderPanel.setAttribute('aria-hidden', 'true');
    document.body.style.overflow = '';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  logoutBtn?.addEventListener('click', () => {
    ...
    brandLink.innerHTML = `Phani <span>Ram</span>`;
